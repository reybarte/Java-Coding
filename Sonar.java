...
public class Sonar {
	...
	int run() throws javax.jms.JMSException
	{
		...
		try {
			...
			Destination dest = sx.createQueue(pqueue);    		// ADC, must use session.createXXXXXX
			MessageConsumer cons = sx.createConsumer(dest);	// ADC, must use session.createConsumer
			...
			while (true) {        // NOC: while loop as is
				...
				if (msg != null) {
					...
					System.out.println(get_time() + " " + boat + ": " + order);
				}
